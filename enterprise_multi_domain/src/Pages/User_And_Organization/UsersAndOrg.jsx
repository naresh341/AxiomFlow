  ...
  return (
    <div className="min-h-screen bg-[#f6f7f8] dark:bg-[#0f172a] text-gray-900 dark:text-white relative">
      <div className="mx-auto px-3 sm:px-6 py-4 sm:py-8">
        {/* HEADER */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 sm:pb-6 animate-in fade-in slide-in-from-top-2 duration-300">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
              {current.buttons.map((btn, i) => (
                <button
                  ...
                  className={`cursor-pointer px-3 sm:px-4 py-2 rounded-lg font-semibold flex items-center gap-2 text-xs sm:text-sm transition ${
                    btn.primary
        ...
        {/* NAVIGATION */}
        <div className="flex overflow-x-auto whitespace-nowrap custom-scrollbar gap-4 sm:gap-8 border-b border-gray-200 dark:border-gray-800 mt-2 sm:mt-4">
          {tabs.map((tab) => (
            <NavLink
              key={tab.path}
              to={tab.path}
              className={({ isActive }) =>
                `pb-3 sm:pb-4 text-sm sm:text-lg font-semibold border-b-2 transition ${
                  isActive
